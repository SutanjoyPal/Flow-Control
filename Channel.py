...
def process_packet(packet: str):
    ...
    if(flag >= 0 and flag < 65):
        ...
        print("Packet delayed due to channel")
        time.sleep(1)
        return packet
    else:
        return ''
...
class ConnectionHandler (Thread):
    def __init__(self, clientSocket, clientAddress) -> None:
        ...
        print(f'\nReceived a new connection from {clientAddress}')
    ...
    def setConnection(self):
        ...
        if (len(availableClients) == 0):
            self.csocket.send(bytes("No client is available currently!", 'utf-8'))
        else:
            ...
            if (client_map[raddr][2] is None):
                ...
                print(self.caddr, "is sending data packets to", raddr)
            else:
                print("Receiver is busy currently!")
            my_lock.release()
    ...
    def revokeConnection(self) -> None:
        ...
        self.csocket.send(str.encode("Sending data packets completed"))
        print(self.caddr, ' ended transferring data packets to ', raddr)
        my_lock.release()
